...
description: Детальный план сбора и подготовки статей — контракт адаптера, оркестрация Celery, дедуп/новизна, декомпозиция T1–T15.
status: stable
tags: [tech, ingestion, plan]
generated:
...
> Детальный декомпозированный план сбора и подготовки статей. Проектировался с прицелом на то, чтобы ни один пункт будущего роадмапа (M2-M6, фазы 1.5/2/3, мультиязычность) не упирался в блокер, заложенный сейчас. Опорные доки: [архитектура](/architecture.md) (стек, адаптеры, cost-metering), [MVP-спека](/mvp-spec.md) §7 (скоуп и acceptance), [AI-пайплайн](/ai-pipeline-prompts.md) (шов к M2).
...
Формальный критерий (AC-1 из [MVP-спеки](/mvp-spec.md)):
...
Extract - отдельная не-LLM стадия (LLM-обогащение `ExtractedArticle` объединено со скорингом в M2, см. [AI-пайплайн](/ai-pipeline-prompts.md)). Цель: чистое полное тело в markdown, язык, медиа.
...
Этика ([архитектура](/architecture.md) §8): robots.txt до fetch (`capabilities.respects_robots`), identifiable UA из settings, per-host throttle, crawl-delay. Тело - для внутреннего пайплайна, выход M3 трансформирует, а не копирует.
...
Оценка совпадает с ориентиром [MVP-спеки](/mvp-spec.md) §7 для M1 (1.5-2 недели на 1-2 инженеров): критический путь T1 -> T5 -> T7 -> T12 -> T15.
