---
type: Handoff
title: Handoff — снимок состояния проекта
description: Что готово и развёрнуто, что нужно от владельца, как запускать локально; инженерный лог вех M0–M6.4.
status: draft
tags: [project, status, handoff]
generated:
...
Прод и stage работают на одной VM (GCP, Debian 13, `35.223.85.130`) под CI/CD. Полный спек инфраструктуры и гочи запуска — [спек деплоя](/deployment.md).
...
Веха M1 реализована целиком по плану [план M1](/plans/m1-ingestion-plan.md) (все 15 задач T1-T15). Полный вертикальный срез `fetch -> normalize -> novelty -> persist -> extract -> dedup` работает, AC-1 доказан интеграционным тестом на реальном Supabase.
...
См. также [MVP-спека](/mvp-spec.md) §7 и матрицу расширяемости [план M1](/plans/m1-ingestion-plan.md) §10.
...
Параллельный трек подготовки к боевому запуску (не путать с «M6 — пилот LOOTON» выше — это про данные пилота). Детальный план, решения и pre-deploy чеклист — [план запуска](/plans/launch-plan.md). В плане работы пронумерованы M6.0-M6.3. Проверено локально: **API 154 unit-теста + ruff (lint+format), web tsc + lint + 68 тестов**. Миграции: `0007_reliability.sql`, `0008_billing.sql`, `0009_email.sql`. Integration-тесты (нужен Supabase-стек) и живой e2e (Stripe/Resend с ключами) — на этапе деплоя.
