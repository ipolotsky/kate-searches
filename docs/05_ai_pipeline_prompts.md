...
class DraftPost(BaseModel):
    ...
    keywords: list[str]
    entities: list[str] = Field(description="люди/бренды/продукты/места явно")
    linked_products: list[dict] = Field(description="привязанные товары [{name, url, why}]")
...
Это НЕ пересказ новости. Найди органичные точки пересечения новости с тем,
что бренд продаёт/делает, и добавь собственную экспертизу/угол бренда
({unique_angle_hint} — напр. ресейл/архив/винтаж/редкость/стоимость).
...
Если есть каталог товаров — предложи 2-5 релевантных товаров с обоснованием привязки.
...
Текст новости: {body}
Товары бренда (если есть): {product_context}
