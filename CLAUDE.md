...
- **Товары клиента:** загрузка через CSV (нет доступа к каталогам клиентов в MVP).
...
Полный decision log — `docs/00_README.md`. Конкуренты/рынок — `docs/01_market_research.md`. Продукт — `docs/02_PRD.md`. Архитектура — `docs/03_architecture.md`. MVP-скоуп — `docs/04_mvp_spec.md`. Промпт-спеки — `docs/05_ai_pipeline_prompts.md`. Экономика — `docs/06_pricing_unit_economics.md`.
...
├── .github/workflows/   # CI (lint+test) и deploy (needs: ci)
...
- **Flowbite MCP:** локально в докере крутится `flowbite-mcp-pro-100`. Чтобы Claude Code его использовал — добавить в `.mcp.json` (шаблон: `docs/mcp.example.json`).
...
2. M1 — ingestion: RSS+sitemap+Crawl4AI адаптеры, extract+dedup+novelty.
